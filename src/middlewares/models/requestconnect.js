...
With an index, it tells you exactly which pages to go to → much faster.
...
// model should be created in capital lettre
const Requestconnect = new mongoose.model("Requestconnect",connectionSchema)
module.exports = Requestconnect*/
...
const connectionSchema = new mongoose.Schema({
  ...
  statuss: {
    type: String,
    enum: { // enum for allowed status values
      values: ["ignored", "pending", "rejected", "accepted"],
      message: `{VALUE} is not allowed`, // fixed typo {values} -> {VALUE}
    }
  }
}, { timestamps: true });

// ✅ Optional: create an index for faster queries (note: firstName and lastName are not fields here, likely typo)
// connectionSchema.index({ firstName: 1, lastNmae: 1 }); 
// removed because these fields do not exist in this schema

// ✅ Pre-save hook to prevent sending request to self
connectionSchema.pre("save", function(next) {
...
const Requestconnect = mongoose.model("Requestconnect", connectionSchema);
module.exports = Requestconnect;
